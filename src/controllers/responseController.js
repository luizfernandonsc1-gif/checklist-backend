...
exports.listAll = async (req, res) => {
  try {
    const responses = await db.query(`
      SELECT r.*, c.title as checklist_title, u.name as loja_name, u.email as loja_email
...
exports.listMine = async (req, res) => {
  try {
    const responses = await db.query(`
      SELECT r.*, c.title as checklist_title
      FROM checklist_responses r
      JOIN checklists c ON c.id = r.checklist_id
      WHERE r.loja_id = ?
      ORDER BY r.id DESC
...
exports.getMyResponse = async (req, res) => {
  try {
    const response = await db.queryOne(`
      ...
      WHERE checklist_id = ? AND loja_id = ?
...
exports.save = async (req, res) => {
  try {
    ...
    let response = await db.queryOne(
      'SELECT * FROM checklist_responses WHERE checklist_id = ? AND loja_id = ?',
      [checklistId, lojaId]
    );

    if (!response) {
      const result = await db.run(
        'INSERT INTO checklist_responses (checklist_id, loja_id, status) VALUES (?, ?, ?)',
        [checklistId, lojaId, 'pending']
      );
